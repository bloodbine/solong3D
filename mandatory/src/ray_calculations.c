...
/*   Updated: 2023/08/15 03:35:24 by ffederol         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_tilePos(t_player *player, t_raycaster *rc)
{
	...
	if (rc->side % 2)
		rc->tilePos = fmod(player->pos->y + rc->ray.y * rc->camPlane2wallDist, 1);
	else
		rc->tilePos = fmod(player->pos->x + rc->ray.x * rc->camPlane2wallDist, 1);
}

// int y = 0;
// 	while (y < screenHeight)
// 	{
// 		mlx_put_pixel(image, rc->x_cam, y, (uint32_t) (rc->texture->pixels[((y * rc->texture->width) + rc->x_cam) * 4]));
// 		//mlx_put_pixel(image, rc->x_cam, drawStart, color);
// 		y++;
// 	}